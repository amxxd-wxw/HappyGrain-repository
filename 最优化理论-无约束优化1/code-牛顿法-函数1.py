...
def conjugate_gradient_direction():
    ...
    while True:
        ...
        hessian_matrix_k_next = torch.autograd.functional.hessian(function1, x_k_next)
        hessian_matrix_list.append(hessian_matrix_k_next)
        #开始求解第k+1步的下降方向d_k_next

        #修正牛顿法：如果海瑟矩阵不正定，那么将下降方向改为负梯度方向
        try:
            L = torch.linalg.cholesky(hessian_matrix_k)
            L_inv = torch.linalg.inv(L)
            d_k = torch.matmul(L_inv, g_k_next)
        except RuntimeError:
            d_k = -g_k_next
...
hessian_matrix_list = []
hessian_matrix_list.append(torch.autograd.functional.hessian(function1, x))   #存储海森矩阵的初始值
