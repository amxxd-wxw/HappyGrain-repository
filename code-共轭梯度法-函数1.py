...
###-------------------------共轭梯度方法---------------------------
def calculate_d_k(g_k, g_k_next, d_k):
    "用于计算共轭梯度法中的下降方向d_k。采用了Fletcher-Reeves公式"
    # 计算分子和分母
    term1 = torch.dot(g_k_next, g_k_next)
    term2 = torch.dot(g_k, g_k)

    if term2 == 0:
        raise ValueError("下降方向为零，无法更新H_k。")
    beta_k = term1 / term2
    d_k_next = -g_k_next + beta_k * d_k
...
def conjugate_gradient_direction():
    ...
    while iter_num < 1000:
        ...
        d_k_next = calculate_d_k(g_k, g_k_next, d_k)
...
plt.title("conjugate_gradient_direction f1 ")
plt.xlabel("Index")
